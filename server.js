    ...
    if (unusedCards.length < req.params.count) {
        ...
        for (var i = 0; i < req.params.count; i++) {
            ...
            while (true) {
                ...
                index = unusedCards[randomValue];
                unusedCards.splice(randomValue, 1); // remove
                // keep going until found a valid type
                if (validTypes.includes(currentCards[index].type)) {
                    break;
                }
            }
